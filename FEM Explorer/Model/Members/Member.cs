        ...
        private void CreateSegments()
        {
            lock (thisLock)
            {
                ...
                while (subSectionsIteration < totalSegments)
                {
                    ...
                    NextNode = Model.Nodes.GetOrAdd(new Tuple<decimal, decimal>(PosX, PosY), new Node(nextNodeIndex, new FEM_Explorer.Point(PosX, PosY, 0), new Codes(), new Constraints(ConstraintType.Free), new NodalLoad(0, 0, 0), false));
                    nextNodeIndex++;
